...
1. v0.5.1: approval ceremony removed. The first legitimate build start auto-seals the run (binding_method=build_start, binding_kind=execution_seal). Legacy tty_confirmation remains for backward-compatible / optional strict pre-seal.
...
Current release line: **0.5.1** (canonical sources: `.claude-plugin/plugin.json` `version` and `lib/ownframework_loop/__init__.py` `__version__`).
