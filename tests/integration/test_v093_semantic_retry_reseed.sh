...
IDEMPOTENT="$(ORDER_JSON="$ORDER" python3 -c 'import json,os; from ownframework_loop import dispatch; print(json.dumps(dispatch.reseed_semantic_artifact_for_retry(json.loads(os.environ["ORDER_JSON"]),previous_attempt_id="builder-attempt-1"),sort_keys=True))')"
assert_eq "$(printf '%s' "$IDEMPOTENT" | jq -r '.already_reseeded')" "true" "completed builder reseed is idempotent"

python3 - "$SEM" <<'PY'
import json,sys
from pathlib import Path
p=Path(sys.argv[1]); p.write_text(json.dumps({"third":"unrelated-payload"})+"\n")
PY
...
assert_eq "$COLLISION" "DispatchError" "contradictory reseed collision fails closed"

python3 - "$REPO" "$RUN" <<'PY'
import sys
from pathlib import Path
from ownframework_loop import build_agent
build_agent.write_skeleton(Path(sys.argv[1]), sys.argv[2], overwrite=True)
PY
...
RIDEMPOTENT="$(RORDER_JSON="$RORDER" python3 -c 'import json,os; from ownframework_loop import dispatch; print(json.dumps(dispatch.reseed_semantic_artifact_for_retry(json.loads(os.environ["RORDER_JSON"]),previous_attempt_id="review-attempt-1"),sort_keys=True))')"
assert_eq "$(printf '%s' "$RIDEMPOTENT" | jq -r '.already_reseeded')" "true" "completed reviewer reseed is idempotent"
...
p=Path(sys.argv[1]); p.write_text(json.dumps({"third":"unrelated-payload"})+"\n")
PY
RCOLLISION="$(RORDER_JSON="$RORDER" python3 -c 'import json,os; from ownframework_loop import dispatch
try: dispatch.reseed_semantic_artifact_for_retry(json.loads(os.environ["RORDER_JSON"]),previous_attempt_id="review-attempt-1")
except Exception as e: print(type(e).__name__)')"
assert_eq "$RCOLLISION" "DispatchError" "contradictory reviewer reseed collision fails closed"

python3 - "$REPO" "$RUN" <<'PY'
import sys
from pathlib import Path
from ownframework_loop import assessment
assessment.write_skeleton(Path(sys.argv[1]), sys.argv[2], overwrite=True)
PY
