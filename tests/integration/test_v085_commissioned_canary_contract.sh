...
assert "pass the required validation" in cps[0]["scope"]
assert "without adding a failing validation test" in cps[0]["scope"]
assert "passes required validation" in meta["acceptance_criteria"][0]["text"]
