...
- These intent rules apply whether this skill was invoked explicitly or the
  operator used a plain-language prompt asking Claude to create a Loop packet.
...
5. Draft WORK_PACKET.md using the repository schema and packet conventions.
   ...
   `browser.playwright.chromium`). Packets never carry raw Claude flags.
...
funded, the supervisor also narrows each Claude print-mode pass with the exact
remaining durable run budget; the Loop ledger remains the canonical aggregate
accounting source.
